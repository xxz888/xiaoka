...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        self.doneBtn = ({
            ...
            [btn setBackgroundImage:[[UIImage imageNamed:@"KD_BindCardBtn"] stretchableImageWithLeftCapWidth:15 topCapHeight:10] forState:UIControlStateNormal];
            [btn setBackgroundImage:[[UIImage imageNamed:@"KD_BindCardBtn"] stretchableImageWithLeftCapWidth:15 topCapHeight:10] forState:UIControlStateSelected];
    ...
    for(int i = 0; i < kTotalNum; i++) {
        ...
        if (i < [_currentMonthDaysArray count] && btn) {
            if ([_selectArray containsObject:interval]) {
                [btn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
                [btn setBackgroundColor:[UIColor hexColorWithString:@"FF9426"]];
...
- (void)doneBtnClick:(id)sender {
    if (_complete) {
        if (self.minDay <= _selectArray.count) {
            ...
            [XHToast showBottomWithText:@"您选择的时间小于计划最小天数" duration:3.0];
        }
    }
