//
//  HCReimbursementFenRunController.m
//  HC
//
...
#import "HCSwipeFenRunCell.h"
#import "HCSwipeFenRunView.h"
@interface HCReimbursementFenRunController ()<UITableViewDelegate, UITableViewDataSource ,DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>{
    NSInteger ThisPage;
}
...
@implementation HCReimbursementFenRunController
...
- (void)getLoadData{
    ...
    [self NetWorkingPostWithURL:self hiddenHUD:NO url:@"/api/user/get/balance/profit/list" Params:dic success:^(id  _Nonnull responseObject) {
        if ([responseObject[@"code"] integerValue] == 0) {
            NSArray * array = responseObject[@"data"][@"list"];
            
            self.DayRevenue_number_lab.text = [NSString stringWithFormat:@"%.2f",[responseObject[@"data"][@"todayBalanceTotal"] doubleValue]];
            self.MonthRevenue_number_lab.text = [NSString stringWithFormat:@"%.2f",[responseObject[@"data"][@"curMonthBalanceTotal"] doubleValue]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString *Identifier = @"HCSwipeFenRunCell";
    HCSwipeFenRunCell *cell = [tableView dequeueReusableCellWithIdentifier:Identifier];
    if(!cell){
        cell=[[[NSBundle mainBundle] loadNibNamed:@"HCSwipeFenRunCell" owner:nil options:nil] lastObject];
    }
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    HCSwipeFenRunView *HeaderView = [[[NSBundle mainBundle] loadNibNamed:NSStringFromClass([HCSwipeFenRunView class]) owner:nil options:nil] lastObject];
